...
\section{Prototype Outputs and Interpretation}
The prototype produces several artifacts to make results transparent:
\begin{itemize}[leftmargin=*]
    \item \textbf{Notebook outputs (demo/):} The notebook loads data (real or synthetic), trains a reward model, simulates RLPF-style selection, and plots a histogram of reward improvements. The histogram shows how often the selected variation improves the reward score over the original text, and the distribution of improvement magnitudes.
    \item \textbf{Run summary JSON (logs/summary\_*.json):} A compact aggregate report with:
    \begin{itemize}[leftmargin=*]
        \item \texttt{count}: number of evaluated texts.
        \item \texttt{mean\_original}: average reward of the original texts.
        \item \texttt{mean\_best}: average reward of the best selected variation.
        \item \texttt{mean\_delta}: mean improvement (\texttt{best - original}).
        \item \texttt{median\_delta}: median improvement.
        \item \texttt{improvement\_rate}: fraction of cases where the selected text scores higher than the original.
    \end{itemize}
    \item \textbf{Agent trace JSON (logs/agent\_runs\_*.json):} Two example runs showing the original text, generated variations, reward scores, and the selected best text. This is a qualitative check that the selection behavior is sensible.
    \item \textbf{Ads CSV (logs/ads\_*.csv):} A snapshot of the processed dataset with text, proxy scores, and metadata. This can be used for further analysis or auditing.
\end{itemize}
